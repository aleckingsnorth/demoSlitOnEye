function drawOnCanvas(angleOfSlit, xVal, yVal, slitHeight, slitWidth, filterOption, blur, magScale) {
    const height = 300;
    const width = 300;
    const rectWidth = (width / 2) * (angleOfSlit + (xVal));
    var scale = getMagScaleFromString(magScale);
...
function getMagScaleFromString(str){
    if(str == "1x"){
        return 1;
    }
...
function chooseMag(oldMagAngle, newMagAngle, currentMag){
    if(Math.abs(oldMagAngle-newMagAngle)<1){
        return currentMag;
    }

    var isIncreasing = isMagAngleIncreasingOrDecreasing(oldMagAngle,newMagAngle);

    if(currentMag == "1x"){

        return isIncreasing ? "1.5x" : "2x";
    }

    if(currentMag == "1.5x"){
        return isIncreasing ? "2x" : "1x";

    }

    if(currentMag == "2x"){
        return isIncreasing ? "1x" : "1.5x";
    }

    return "1x"
}

function isMagAngleIncreasingOrDecreasing(oldValue, newValue) {
...
function getBlurValueFromBaseZ(baseZ) {
    var zMin = -0.76;
    var zMax = 0.16;
    var maxBlur = 10;
    var midPoint = (zMin + zMax) / 2;
    var halfRange = (zMax - zMin) / 2;
    var percent = Math.abs(baseZ - midPoint) / halfRange;
    var blur = Math.round(percent * maxBlur);
    return blur;
}
